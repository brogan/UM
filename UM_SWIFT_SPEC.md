...
The right panel exposes per-layer driver UI in the **LAYER DRIVERS** section (collapsed by default). **Blend, Opacity, and Offset apply to both grid and open layers.** The section begins with a **Blend** picker (`UMBlendMode`: Normal, Multiply, Screen, Overlay, Dodge, Burn, Soft Light, Hard Light, Difference, Exclusion, Add), then OPACITY and OFFSET driver subsections. Both `opacityDriver` and `layerOffset` support oscillator, jitter, noise, and keyframe modes — see §15.8. The **DISTORTION** subsection is **grid-only** and is hidden when the active layer is `.sprite` (guarded by `ls?.layerMode != .sprite` in `layerDriversSection`). The RENDER section title changes dynamically to **STYLE — [name]** when a style is active. A **SHAPE — [name]** section appears below MOTION when a shape palette item is active, showing polygon counts and cell usage in the active layer. A contextual **"Nothing active"** hint appears at the bottom when no KF, motion, cell selection, or shape is active.
...
- **Loop playback button** ✓ Built 2026-06-25 — `repeat` icon, accent-coloured when `isLooping == true`. `AppController.isLooping: Bool`. Wired into the playback loop: after `ls.engine.advance()`, if `isLooping && engine.currentFrame >= endFrame`, calls `seekToFrame(startFrame)`. S/E fields serve as both the playback loop window and the export render range.
- **⌘-drag ruler to sweep region** ✓ Built 2026-06-25 — `commandDown: Bool` computed var (NSEvent.modifierFlags). New `TLDragKind.regionDrag`. When drag starts in the ruler area with Cmd held, anchor frame = drag start X, cursor frame = drag current X; `startFrame = min(anchor, cursor)`, `endFrame = max(anchor+1, cursor)` updated live. Tap with Cmd seeks to `startFrame`. Plain ruler drag continues to scrub the playhead.
- **Timeline resize preview line** ✓ Built 2026-06-25 — `AppController.timelineResizePreviewH: CGFloat?`. Set in `.onChanged` of the resize handle gesture, cleared in `.onEnded`. `ContentView` overlays a 2px accent `Rectangle` positioned `resizePreviewH` px from the bottom of the VStack — floats over the canvas with `allowsHitTesting(false)`, no layout changes during drag. `panelHeight` still only commits on release.
...
- **Direct sprite set drawing** ✓ Built 2026-06-25 — `AppController.activeAnimatedGeometryID: UUID?`. Tapping a Sprite Set row in the left palette sets/clears this ID; the row highlights in accent colour. `addSprite(at:)` assigns `sprite.animatedGeometryID = activeAnimatedGeometryID` so every placed sprite inherits the active Sprite Set without a separate inspector step.

---

### 15.12 Open Layer (formerly Sprite Layer) ✓ Built 2026-06-20 (Phase 2 ✓ Built 2026-06-20; renamed 2026-06-25)
...
| Animation | Phase, motion set, path, sequence, **Sprite Set** | Phase + motion set, Position Driver, **Sprite Set** |
| Layer drivers | Blend, Opacity, Offset, Distortion | Blend, Opacity, Offset (Distortion hidden) |
...
- `+ New Layer` button replaced by a `Menu` with two items: **Grid Layer** and **Open Layer** (internally `LayerMode.sprite`, renamed in UI only).
- Open layer rows show a `sparkles` icon between the accent dot and the name to distinguish them from grid layers.
- All existing layer controls (visibility, opacity, parallax, rename, reorder, context menu) are unchanged.

**QuickAdjustView — open layer mode**
...
- **LAYER DRIVERS on sprite layers**: ✓ Built 2026-06-25. `opacityDriver`, `layerOffset`, and `blendMode` were already evaluated in the sprite branch of the main canvas loop (lines 699–710 of `ContentView.swift`) and in `UMExporter`. The only change was UI: the DISTORTION subsection in `layerDriversSection` is now wrapped in `if ls?.layerMode != .sprite` so sprite layers see Blend / Opacity / Offset only. Sprite layers show opacity + offset lanes in the timeline (no gridScroll lane, no distortion).
...
public struct UMAnimatedGeometry: Codable, Identifiable, Sendable {
    ...
    public var totalForwardFrames: Int   // sum of holdFrames + transitionFrames per state
    public var totalCycleFrames: Int     // includes pingPong reverse pass (hold-only)
    // Primary render API — returns 1 layer (hard cut) or 2 layers (during cross-fade)
    public func resolveRenderLayers(atFrame:) -> [UMRenderLayer]
    // Convenience delegates (return primary layer values)
...
public struct UMAnimatedGeometryState: Codable, Identifiable, Sendable {
    ...
    public var holdFrames: Int           // frames this state is fully shown (min 1)
    public var transitionFrames: Int     // frames to cross-fade into the next state (0 = hard cut)
    public var easing: PathEasing        // easing curve applied during transitionFrames
    public var offsetX, offsetY: Double  // per-state position offset (canvas pixels)
    public var rotation: Double          // per-state rotation offset (degrees)
    public var scaleX, scaleY: Double    // per-state scale multiplier
}

public struct UMRenderLayer: Sendable {
    public let shapeID:   UUID
    public let styleID:   UUID?
    public let alpha:     Double   // draw opacity 0...1; complementary during cross-fade
...
**Grid cell Sprite Set support ✓ Built 2026-06-25**

`UMGridCell` also gains `animatedGeometryID: UUID?` (Codable with `decodeIfPresent` for backward compat). `UMGridEngine.setCellDrawn` and `floodFill` both accept `animatedGeometryID: UUID? = nil` and store it on the cell. `AppController.activeAnimatedGeometryID` (already wired for open-layer sprite placement) is now also passed through to `setCellDrawn`/`floodFill` in the draw tool and flood-fill tool, so drawing on a grid layer while a Sprite Set is active in the palette automatically assigns that Sprite Set to each drawn cell.

`resolveEffectiveCellLayers(cell:motionSet:animatedGeometries:frame:) -> [UMRenderLayer]` in `GridScrollUtils.swift` mirrors `resolveEffectiveSpriteLayers`: if `cell.animatedGeometryID` is set, delegates to `geo.resolveRenderLayers(atFrame: frame + cell.phaseOffset)`; otherwise falls back to `resolveSequenceShapeID` then the cell's own `shapeID`. The main canvas grid cell render loop now iterates `resolvedLayers` exactly as the open-layer sprite render, applying `geoLayer.alpha`, `geoLayer.transform.offsetX/Y`, `geoLayer.transform.rotation`, and `geoLayer.transform.scaleX/Y` per layer. `attemptMorphLayers` is called the same way — vertex morph fires automatically for topology-matching Sprite Sets.

**PLACE & TIME: Anim. Set picker** — `selectionAnimGeoBinding` (get: `focusedCell?.animatedGeometryID`; set: `controller.assignAnimGeoToSelection($0)`) drives a new `Picker("", selection: selectionAnimGeoBinding)` below the existing Shape picker. Lists all `controller.projectAnimatedGeometries` with a `—` (nil) option. `AppController.assignAnimGeoToSelection(_ geoID: UUID?)` applies the value to all selected cell indices with an undo snapshot.
...
| Phase 1 | `UMGridCell.animatedGeometryID` — Sprite Sets on grid cells | ✓ Built 2026-06-25 |
| Phase 1 | Grid canvas render loop uses resolveEffectiveCellLayers + morph/cross-fade | ✓ Built 2026-06-25 |
| Phase 1 | Anim. Set picker in PLACE & TIME (Quick Adjust) with assignAnimGeoToSelection | ✓ Built 2026-06-25 |
...
| Phase 2b | Cross-fade (`transitionFrames > 0`): `UMRenderLayer` + `resolveRenderLayers` | ✓ Built 2026-06-25 |
...
| Phase 4 | Style tweening: `UMAnimatedGeometryState.styleTween`, `UMRenderLayer.styleTween`, `lerpUMColor` at render sites | ✓ Built 2026-06-25 |
| Phase 4 | Style↔ checkbox in editor transition sub-row (dimmed when Trans = 0) | ✓ Built 2026-06-25 |
| Phase 4 | Palette: inline rename (double-tap + context menu), Duplicate (deep copy, inserted after original) | ✓ Built 2026-06-25 |

**Render site integration**

All five render paths call `resolveEffectiveSpriteLayers` / `resolveEffectiveCellLayers` in `GridScrollUtils.swift`, returning `[UMRenderLayer]`. Each site iterates `for geoLayer in geoLayers` and multiplies fill/stroke alpha by `geoLayer.alpha`. During a hard-cut frame: one layer at `alpha = 1.0`. During `transitionFrames`: two layers at complementary alphas `(1 − progress, progress)` with `PathEasing` applied.

**Style tweening** (`UMRenderLayer.styleTween` ✓ Built 2026-06-25): `UMAnimatedGeometryState` carries `styleTween: Bool = false` (Codable, `decodeIfPresent`, backward compat). `resolveRenderLayers(atFrame:)` copies `primary.styleTween` onto both returned layers during a transition. At each render site, before the layer loop:

```swift
let tweenActive = geoLayers.count == 2 && geoLayers[0].styleTween
let tweenT      = tweenActive ? geoLayers[1].alpha : 0.0
let tweenFill   = tweenActive ? lerpUMColor(fillA, fillB, t: tweenT) : nil
let tweenStroke = tweenActive ? lerpUMColor(strokeA, strokeB, t: tweenT) : nil
```

`fillA`/`fillB` are the FROM/TO states' effective fill colours (styleID lookup → fallback to sprite or cell base style). The blended colour is used for all resolved layers in the loop, replacing per-layer style lookup. This applies equally to cross-fade (two layers at complementary alphas) and morph (single merged layer at alpha 1.0), giving continuous colour interpolation in both cases. `lerpUMColor(_ a: UMColor, _ b: UMColor, t: Double) -> UMColor` is a file-private helper in ContentView.swift that lerps all four channels linearly.

The morph synth layer additionally has `styleTween` propagated from `geoLayers[0].styleTween` so the style-tween path fires correctly when vertex morph is also active.

The convenience functions `resolveEffectiveSpriteShapeID`, `resolveEffectiveSpriteStyleID`, and `resolveEffectiveSpriteStateTransform` delegate to the first render layer and are preserved for hit-testing and non-render callers.
...
- State list (scrollable): each row has ▸ expand toggle, active-state indicator dot, shape picker, style override picker, Hold frames field, − delete, ↑↓ reorder. Expanded: two-row transform sub-row — row 1: Δx, Δy, °, Sx, Sy; row 2: **Trans** (transitionFrames int) + **Ease** (PathEasing picker, dimmed when Trans = 0) + **Style↔** (styleTween checkbox, dimmed when Trans = 0)
- Live preview canvas (160px, dark background): renders via `resolveRenderLayers(atFrame:)` — shows cross-fade and style tween between states during transition windows; alpha composited at `fC.a * layer.alpha`; style-tween blended colours applied when active
- Scrubber: ▶/⏸ play button (24fps), slider over full `totalCycleFrames`, frame counter, active shape name

**Palette management (✓ Built 2026-06-25)**

- **Rename** — inline `TextField` shown when `renamingGeoID == geo.id` (same pattern as shapes/motions). Triggered by double-clicking the name or via context menu → Rename. Press Return to confirm.
- **Duplicate** — `AppController.duplicateAnimatedGeometry(id: UUID)`: deep copies the `UMAnimatedGeometry` struct (new `UUID`, new `id`, name prefixed "Copy of"), inserts it at `idx + 1` in `projectAnimatedGeometries`. All states including `styleTween`, `transitionFrames`, and per-state transforms are copied. The duplicate is fully independent.
- Context menu now: Edit…, Rename, Duplicate, [divider], Delete.
...
**Resolved design decisions (Phase 4 — Style Tween)**

- Style tween is per-state (controls the transition *out of* that state). A cycle can mix tweened and hard-cut transitions freely.
- When `styleTween = true` and one or both states have no style override (`styleID = nil`), the element's own base style colour is used as that end of the interpolation. No special-case handling needed at the render site; the standard style lookup (`geoLayer.styleID ?? sprite.styleID`) resolves it naturally.
- `styleTween` is stored on `UMRenderLayer` (not just `UMAnimatedGeometryState`) so render sites do not need to re-query the asset. Both cross-fade layers carry the same flag; only `geoLayers[0].styleTween` is checked at the render site since both are always equal during a transition.
- The morph synth layer (`resolvedLayers = [synth]` when `attemptMorphLayers` succeeds) propagates `styleTween` from `geoLayers[0]` so the style-tween path fires correctly even when the geometry is merged into a single layer.
- Colour interpolation uses linear (per-channel) lerp on all four RGBA channels. No gamma-aware or perceptual interpolation at this stage.
...
| **Open layer** | Free-placed sprites with per-polygon color overrides | ✓ Built 2026-06-20 |
| **Open layer** | MOTION section in sprite context (effectiveMotionID from selected sprite) | ✓ Built 2026-06-20 |
| **Open layer** | positionDriver keyframe lane in timeline (per-sprite purple lanes, full KF ops) | ✓ Built 2026-06-20 |
| **Open layer** | LAYER DRIVERS (Blend, Opacity, Offset) — render path already wired; Distortion hidden in UI | ✓ Built 2026-06-25 |
| **Open layer** | Direct drawing with active Sprite Set (tap palette row to activate; auto-assigned on place) | ✓ Built 2026-06-25 |
| **Open layer** | Polygon override index stability (UUID-keyed instead of positional) | ✓ Built 2026-06-20 |
| **Open layer** | UI rename: "Sprite Layer" → "Open Layer" (LayerMode.sprite kept internally) | ✓ Built 2026-06-25 |
| **Sprite Set** | Phase 1–3: image replacement, per-state transforms, cross-fade, vertex morph | ✓ Built 2026-06-25 |
| **Sprite Set** | Phase 4: Style Tween (UMAnimatedGeometryState.styleTween, lerpUMColor at all render sites, Style↔ checkbox) | ✓ Built 2026-06-25 |
| **Sprite Set** | Palette: inline rename (double-tap / context menu), Duplicate (AppController.duplicateAnimatedGeometry) | ✓ Built 2026-06-25 |
| **Grid layer** | Sprite Sets on grid cells: UMGridCell.animatedGeometryID, resolveEffectiveCellLayers, Anim. Set picker in PLACE & TIME | ✓ Built 2026-06-25 |
...
| **Transport** | Loop playback (isLooping; wraps endFrame→startFrame; Loop button in transport) | ✓ Built 2026-06-25 |
| **Transport** | ⌘-drag ruler to sweep render/loop region (TLDragKind.regionDrag) | ✓ Built 2026-06-25 |
| **Timeline** | Resize drag preview line (timelineResizePreviewH overlay; no layout change during drag) | ✓ Built 2026-06-25 |
