...
All five render paths now call `resolveEffectiveSpriteLayers(sprite:motionSet:animatedGeometries:frame:)` in `GridScrollUtils.swift`, which returns `[UMRenderLayer]`. Each render site iterates `for geoLayer in geoLayers` and multiplies the fill and stroke alpha by `geoLayer.alpha`. During a hard-cut frame there is one layer at `alpha = 1.0`; during `transitionFrames` there are two layers at complementary alphas `(1 − progress, progress)` with `PathEasing` applied. This mechanism handles both shape cross-fade and style animation in one pass with no explicit colour interpolation.
...
- State list (scrollable): each row has ▸ expand toggle, active-state indicator dot, shape picker, style override picker, Hold frames field, − delete, ↑↓ reorder. Expanded: two-row transform sub-row — row 1: Δx, Δy, °, Sx, Sy; row 2: **Trans** (transitionFrames int) + **Ease** (PathEasing picker, dimmed when Trans = 0)
- Live preview canvas (160px, dark background): renders via `resolveRenderLayers(atFrame:)` — shows cross-fade between states during transition windows; alpha composited at `fC.a * layer.alpha`
...
| **Sprite Set** | Animated Geometry / Sprite Set assets — Phase 1 + Phase 2a (transforms) + Phase 2b (cross-fade) + Phase 3 (vertex morph) | ✓ Built 2026-06-25 |
